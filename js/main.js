...
function generateScreenshotsAuto(max = 100) {
    ...
    videoList.forEach(video => {
        items.push({ type: "video", src: video.src, num: video.num, desc: `Video ${video.num}` });
    });
    for (let i = 1; i <= max; i++) {
        ...
        img.src = path;
        img.onload = () => {
            items.push({ type: "image", src: path, num: i, desc: `Screenshot ${i}` });
...
const infoHTML = `
  <div class="info-accordion">
    <details>
      ...
      <div class="video-wrapper">
        <div class="video-container">
          <iframe src="https://youtu.be/YF2zsJ0L1I8?si=-N6AuLgrOwpX99rb" frameborder="0" allowfullscreen></iframe>
        </div>
      </div>
      <p class="video-description">Guía paso a paso para instalar shaders en Minecraft.</p>
    </details>
    ...
    <details><summary>SPECIAL CREDITS</summary><p>Muchas gracias a @Stellar por crear el servidor de discord y algunos codigos compartidos:] Tambien a @Jesusmiguelgamer por sus texturas noise y sky personalizadas la motivacion y inspiracion :D.</p></details>
...
document.getElementById('profileStats').textContent = '560 Followers · 6 Projects · 112k Downloads';
...
function openShaderDetailModal(shaderId) {
    ...
    // --- LÓGICA DE BOTÓN WINDOWS & IOS ---
    const windowsBtn = document.getElementById('detailWindowsBtn');
    // AÑADE LOS TÍTULOS DE LOS SHADERS QUE QUIERAS AQUÍ:
    const shadersConBoton = ["NEWBXREXYTMC", "NUEVO_SHADER_AQUI"]; 
    
    if (shadersConBoton.includes(shader.title)) {
...
function switchMainScreen(src) {
    ...
    if (src === "video" && youtubeEmbedUrl) {
        mainScreen.innerHTML = `<iframe src="${youtubeEmbedUrl}" frameborder="0" allowfullscreen></iframe>`;
    } else {
        mainScreen.innerHTML = `<img id="mainScreenDetail" src="${src}">`;
    }
    document.querySelectorAll('.screenshots-detail img').forEach(s => s.classList.remove('active'));
    const active = Array.from(document.querySelectorAll('.screenshots-detail img')).find(img => img.onclick.toString().includes(src));
...
function renderShadersList() {
    ...
    content.innerHTML = ''; 
    const filteredShaders = filterShaders(currentSearchTerm).sort((a, b) => b.id - a.id);
    ...
    searchContainer.innerHTML = `<input type="text" id="shaderSearchInput" value="${currentSearchTerm}"><button onclick="handleSearch()">Search</button>`;
    ...
    if (paginatedShaders.length === 0) {
        ...
        paginatedShaders.forEach(shader => {
            const featureIcons = {
                "Vanilla": "⚡", "Cielo 3D": "☁️", "Agua Sutil": "💧", "Efecto 'Lens Flare'": "☀️", "Vanilla-like": "🍦",
                "Iluminación": "💡", "PVP": "⚔️", "Sombreado": "⚫", "Shader": "🌅", "World": "🏞", "Texturas": "🎨", "Addons": "🗂"
            };
            const featuresHtml = shader.features.map(feat => `<div class="shader-feature">${featureIcons[feat] || '🔹'} ${feat}</div>`).join('');
            ...
            <div class="shader-content">
                <div class="shader-icon"><img src="${shader.iconImage || 'assets/placeholder_icon.png'}" alt="Icono del Shader"></div>
                <div class="shader-text"><p class="shader-title">${shader.title}</p><p class="shader-author">By ${shader.author || 'Desconocido'}</p></div>
            </div>
            <div class="shader-description-container">
                <p class="shader-description">${shader.description}</p>
                <div class="shader-features">${featuresHtml}</div>
            </div>
            <div class="shader-version-info">${versionInfoHtml}</div>
    ...
    if (totalPages > 1) {
        ...
        paginationDiv.innerHTML = `<button onclick="setPage(${currentPage - 1})" ${currentPage === 1 ? 'disabled' : ''}>Anterior</button><span class="page-info">Página ${currentPage} de ${totalPages}</span><button onclick="setPage(${currentPage + 1})" ${currentPage === totalPages ? 'disabled' : ''}>Siguiente</button>`;
...
const banners = ['assets/portada.png', 'assets/portada1.png', 'assets/portada2.png', 'assets/portada3.png'];
